...
object and then create the configurable objects, passing the conf to them.
"""
...
base_aliases = {
    ...
    'conf' : 'BaseIPythonApplication.extra_config_file',
}
...
base_flags = dict(
    ...
    init = ({'BaseIPythonApplication' : {
                    ...
                    'auto_create' : True}
            }, """Initialize profile with default conf files.  This is equivalent
            to running `ipython profile create <profile>` prior to startup.
            """)
)

class ProfileAwareConfigLoader(PyFileConfigLoader):
    """A Python file conf loader that is aware of IPython profiles."""
...
class BaseIPythonApplication(Application):
    ...
    builtin_profile_dir = Unicode(
        os.path.join(get_ipython_package_dir(), u'conf', u'profile', u'default')
    )
    ...
    help="""Path to an extra conf file to load.
    
    If specified, load this conf file in addition to any other IPython conf.
    """).tag(config=True)
    ...
    def _profile_changed(self, change):
        self.builtin_profile_dir = os.path.join(
                get_ipython_package_dir(), u'conf', u'profile', change['new']
        )
    ...
    overwrite = Bool(False,
        help="""Whether to overwrite existing conf files when copying"""
    ).tag(config=True)
    ...
    copy_config_files = Bool(False,
        help="""Whether to install the default conf files into the profile dir.
        If a new profile is being created, and IPython contains conf files for that
        profile, then they will be staged into the new directory.  Otherwise,
        default conf files will be automatically generated.
        """).tag(config=True)
    ...
    def _ipython_dir_changed(self, change):
        ...
        readme_src = os.path.join(get_ipython_package_dir(), u'conf', u'profile', 'README')
    ...
    def load_config_file(self, suppress_errors=IPYTHON_SUPPRESS_CONFIG_ERRORS):
        ...
        By default, errors in loading conf are handled, and a warning
        ...
        self.log.debug("Searching path %s for conf files", self.config_file_paths)
        base_config = 'ipython_config.py'
        self.log.debug("Attempting to load conf file: %s" %
                       base_config)
        try:
        ...
        for config_file_name in self.config_files:
            ...
            try:
                ...
                # Only warn if the default conf file was NOT being used.
                ...
                if not suppress_errors:
                    raise
                self.log.warning("Error loading conf file: %s" %
    ...
    def init_config_files(self):
        """[optionally] copy default conf files into profile dir."""
        ...
        if self.copy_config_files:
            ...
            # Still stage *bundled* conf files, but not generated ones
    ...
    def initialize(self, argv=None):
        ...
        if self.subapp is not None:
            ...
            return
        # save a copy of CLI conf to re-load after conf files
