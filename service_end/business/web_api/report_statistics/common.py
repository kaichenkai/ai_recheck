...
def objs_get_result(counts, today_result=None):
    ...
    for count in counts:
        ...
        tmp[count.sector_code].setdefault('report_count', 0)  # 上报量
        ...
        tmp[count.sector_code]['report_count'] += count.report_count
        report_counts += count.report_count     # 上报总量
    ...
    if today_result:
        ...
        for r in today_result.get('result', []):
            ...
            if sector_code not in tmp:
                ...
                tmp[sector_code].setdefault('report_count', 0)  # 上报量
            ...
            tmp[sector_code]['report_count'] += r.get('report_count', 0)  # 上报量
    ...
    for k, v in tmp.items():
        result.append({"name": cons.SECTOR_MAP.get(k, "无名称或未录入"), "insert_count": v.get('insert_count', 0),
                       ...
                       "report_count": v.get('report_count', 0),  # 上报数量
...
def create_results(start_time, end_time, manual_check_status, current=None, pageSize=None):
    ...
    #    v = sector_map.get(i)
    #    x = "{}%".format(i)
    #    tmp = query.filter(WfRecord.office_code.ilike(x))\
    ...
    #               .filter(WfRecord.office_code.ilike(x))\
    #               .with_entities(func.count(WfRecord.id)).scalar()
...
# 导出统计
def create_results_info(start_time, end_time, office_name, manual_check_status, current=None, pageSize=None):
    # current = data.get('current', 1)
    # pageSize = data.get('pageSize', 24)
    # start_time = data.get('start_time', "")
    # end_time = data.get('end_time', "")
    ...
    if not office_name:
        query = query.filter(WfRecord.recog_status == 2) \
            .filter(WfRecord.sdk_reason_code > 0) \
            .filter(WfRecord.car_plate_number != WfRecord.sdk_car_plate_number) \
            .filter(WfRecord.correct_sector_code.in_([code for code in cons.SECTOR_MAP.keys()])) \
            .filter(WfRecord.manual_check_status == 2)
        ...
        query = query.filter(WfRecord.recog_status == 2) \
            .filter(WfRecord.sdk_reason_code > 0) \
            .filter(WfRecord.car_plate_number != WfRecord.sdk_car_plate_number) \
            .filter(WfRecord.office_name == office_name)
        # .filter(WfRecord.office_code.ilike("{}%".format(map_sector.get(name))))
    # 通报只展示车牌更正
    ...
    if manual_check_status and office_name:
...
def get_image_path(file_name):
    return os.path.join(cons.IMAGE_SAVE_PATH, file_name)
